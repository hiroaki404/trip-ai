...
1. **ユーザーが承認した場合（「OK」「いいです」「問題ありません」などの肯定的な回答）**
   ...
   - カレンダー登録が完了したら、nullを出力して終了してください

2. **修正が必要な場合（具体的な変更要求がある場合）**
   - ユーザーの要望に応じて旅行計画を修正します。
   - このタスクでは計画を修正しません。このタスクは追加の要望で終了し、他のタスクに処理を移譲します。
...
- **カレンダーの登録が完了した場合、nullで終了してください**
- **__feedback_user__ツールのsummaryパラメータには旅行計画の要約(summary)を渡してください**
- ユーザーから修正の要望があった場合のみ、追加の要望の文字列で終了し、他のタスクに処理を委譲します。
